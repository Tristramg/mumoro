...
std::pair<size_t, size_t> MultimodalGraph::load_pg(const std::string & layer, const std::string &nodes, const std::string & edges, Mode m)
{
    size_t nb_nodes = load_nodes_pg(layer, nodes, m);
    size_t nb_edges = load_edges_pg(layer, edges, m);
    return std::make_pair(nb_nodes, nb_edges);
}
...
size_t MultimodalGraph::load_nodes_pg(const std::string & layer, const std::string & table, Mode m)
{
    size_t count = 0;
    std::stringstream query;
    query << "SELECT * FROM " << table;

    pqxx::work sql(*conn, "Reading nodes from table " + table);
    pqxx::result R;
    try
    {
        R = sql.exec(query);
    ...
    {
        std::cerr << "La requète pour récupérer les données a échoué, c'était : " << std::endl;
    ...
    for (rit = R.begin(); rit != R.end(); rit++)
    {
        ...
        layers[n.layer][n.id] = new_node;
        layers[n.layer].append(n.lon, n.lat, new_node);
        count++;
    }
    return count;
}

size_t MultimodalGraph::load_edges_pg(const std::string & layer, const std::string & table, Mode m)
{
    size_t count = 0;
    std::stringstream query;
    query << "SELECT * FROM " << table;
    ...
    for (rit = R.begin(); rit != R.end(); rit++)
    {
        ...
        (*rit)["target"].to(target);
        (*rit)["length"].to(length);
        (*rit)["car"].to(car);
        (*rit)["car_rev"].to(car_reverse);
        (*rit)["bike"].to(bike_reverse);
        (*rit)["bike_rev"].to(bike_reverse);
        (*rit)["foot"].to(foot);
...
bool MultimodalGraph::init_pg(const std::string & pg_conn)
{
    conn = new pqxx::connection(pg_conn);
    return conn->is_open();
}
